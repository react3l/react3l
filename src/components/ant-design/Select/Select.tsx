...
const Select = React.forwardRef(<T extends Model, TSearch extends Search>(props: SelectProps<T, TSearch>, ref: Ref<any>) => {
  const {
    search, setSearch, className, list: defaultList, children, getList, onSearchError, allowClear, onChange, searchField, value, defaultValue, render,
  } = props;
  ...
  const handleSearch = React.useMemo(() => debounce((value: string) => {
    setSearch(Search.clone<TSearch>({
      ...search, [searchField]: value,
    }));
  }), [search, searchField, setSearch]);

  const handleToggle = React.useCallback(async (visible: boolean) => {
    if (visible && typeof getList === 'function') {
      await handleLoadList();
    }
  }, [getList, handleLoadList]);

  const handleChange = React.useCallback((value: number | string, option?: ReactElement<SelectOptionProps<T>>) => {
    if (onChange) {
      if (value && option) {
